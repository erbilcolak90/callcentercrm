...
public class TransferService {
    ...
    public Result<Transfer> getById(String transferId){
        ...
        if(authentication.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ADMIN"))){
            return new Result<>("Success", transfer, true);
        }
        if(authentication.getName().equals(transfer.getUserId())){

            return new Result<>("Success", transferRepository.getById(transferId), true);
        }else{
    ...
    public Result<Page<Transfer>> getAllTransfer(PaginationInput paginationInput){
        ...
        if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ADMIN"))) {
            return new Result<>("Success", transferRepository.findAll(pageable), true);
        } else {
            return new Result<>("All transfers",transferRepository.findByIsDeletedFalse(pageable), true);
        }
    ...
    public Result<String> withdrawal(@RequestBody TransferInput transferInput) {
        ...
        if (receiverTransferAccount != null && !receiverTransferAccount.getCurrency().equals(tradingAccount.getCurrency())) {
            return new Result<>("Bank and trading account currencies cannot be differient", null, false);
        }
        if (tradingAccount.getBalance() < transferInput.getAmount()) {
            return new Result<>("transfer amount cannot bigger than trading account balance", null, false);
        }
        ...
        if (authentication.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_ADMIN"))
            ...
            return new Result<>("Success", TransferStatus.IN_PROGRESS.toString(), true);
        } else {
    ...
    public Result<String> updateTransfer(Transfer transferInput) {
        ...
        if (db_Transfer == null || db_tradingAccount == null) {
            return new Result<>("Transfer or trading account not found", null, false);
        }
        ...
        if(transferInput.getReceiverTransferAccountId() != null){
            ...
            if(receiverTransferAccount == null){
                return new Result<>("Receiver transfer account not found",null, false);
            }
