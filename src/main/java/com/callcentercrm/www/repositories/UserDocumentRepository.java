...
public interface UserDocumentRepository extends MongoRepository<UserDocument,String> {
    ...
    @Query(value = "{'userId': ?0}",fields = "{'id': 1, 'userId':1 , 'documentType': 1, 'frontURL': 1, 'backURL': 1, 'status': 1, 'statusMessage': 1, 'createDate': 1, 'updateDate':1  }")
    UserDocument findByUserId(String userId);

    @Query(value = "{'id': ?0, 'userId': ?1 }",fields = "{'id': 1, 'userId':1 , 'documentType': 1, 'frontURL': 1, 'backURL': 1, 'status': 1, 'statusMessage': 1}")
